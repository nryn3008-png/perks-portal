...
export function VendorIcon({
  ...
  return (
    <div
      className={`flex items-center justify-center rounded-xl bg-slate-100 ${config.container} ${className}`}
      aria-hidden="true"
    >
      {showLogo && (
        <Image
      ...
      {showInitial && (
        <span className={`font-semibold text-slate-400 ${config.initial}`}>
          {initial}
        </span>
      )}
    </div>
