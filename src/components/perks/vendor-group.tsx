...
/**
 * Placeholder Gift Icon - matches OfferCard fallback
 */
...
/**
 * Vendor Logo for group header - 40x40 with fallback
 */
function VendorGroupLogo({ src }: { src: string | null }) {
  ...
  if (!src || error) {
    return (
      <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#ecedf0]">
        <PlaceholderGiftIcon />
      </div>
    );
  }
  ...
  return (
    <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-[#ecedf0]">
      <Image
        src={src}
        alt=""
        width={40}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      ...
      <button
        ...
        className="flex w-full items-center gap-4 p-4 text-left transition-colors hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500"
      >
        {/* Vendor Logo */}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-base font-bold leading-[22px] text-[#0d1531]">
              {vendorName}
            </h3>
          ...
          {vendorPrimaryService && (
            <p className="truncate text-sm leading-5 tracking-[0.4px] text-[#81879c]">
              {vendorPrimaryService}
            </p>
          )}
        </div>
